...
`mailscript`) выполняются от имени этого пользователя (аналог
`switch_user()` в оригинале — Windows-порт выполняет вход через
`LogonUser` и имперсонацию).

Обратите внимание:

- пароль хранится в открытом виде в конфигурационном файле;
- вход под другим пользователем требует, чтобы logrotate был запущен
  с правами администратора (SeImpersonatePrivilege), иначе `LogonUser`
  вернёт ошибку `1314` и скрипты выполнятся от текущего аккаунта
